...
export default function InstantQuotePage() {
  ...
  const onSubmit = async (data: QuoteFormData) => {
    setResult(null);
    const res = await submitQuote({ ...data, sourcePage: "/instant-quote", turnstileToken: turnstileToken ?? undefined });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <Breadcrumb items={[{ label: "Instant Quote" }]} />
      <h1 className="mt-6 text-3xl font-semibold tracking-tight">Instant Quote</h1>
      <p className="mt-2 max-w-2xl text-muted-foreground">
        Fill in your packaging specifications below and receive a preliminary estimate within
        2 working hours (GMT+8).
      </p>
      ...
      <div className="mt-8 rounded-lg border bg-card p-6 sm:p-8">
        {result ? (
          ...
          >
            ...
            {!result.ok && (
              <Button variant="link" size="sm" className="mt-2" onClick={() => setResult(null)}>
                Try again
              </Button>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              {errors.boxType && <p className="text-sm text-destructive">{errors.boxType.message}</p>}
            </div>
